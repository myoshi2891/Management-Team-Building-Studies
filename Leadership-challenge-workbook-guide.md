...
LPIは、Kouzes & Posner が開発した30項目の行動アンケートによる360度評価ツールで、5つの実践それぞれに対応する6つの行動の実践頻度を、自己評価と周囲（上司・同僚・部下など）からの評価の両面から測定します。公式サイトによれば、このモデルとLPIは40年以上の実証研究、5,000件以上のケーススタディ、750件以上の独立研究（independent research studies）に裏付けられており、70カ国以上で用いられ、開発以来800万人超の回答者（LPI利用者）のデータが蓄積されています。

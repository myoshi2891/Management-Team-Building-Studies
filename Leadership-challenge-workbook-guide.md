...
LPIは、Kouzes & Posner が開発した30項目の行動アンケートで、5つの実践それぞれに対応する6つの行動の実践頻度を測定します。周囲（上司・同僚・部下など）の観察者による評価を集める「LPI 360」と、自己評価のみで完結する自己診断版がそれぞれ提供されており、両者は別のものです。自己評価と観察者評価を突き合わせたギャップの分析は、観察者評価を伴う LPI 360 で行います。公式サイトによれば、このモデルとLPIは40年以上の実証研究、5,000件以上のケーススタディ、750件以上の独立研究（independent research studies）に裏付けられており、70カ国以上で用いられ、開発以来800万人超の回答者（LPI利用者）のデータが蓄積されています。

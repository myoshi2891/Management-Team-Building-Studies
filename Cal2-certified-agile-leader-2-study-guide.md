...
RAPIDは実行順序を定めたプロセスではなく、意思決定に関わる5つの役割を切り分けるための枠組みです。Input（情報提供）・Recommend（提案）・Agree（合意）・Decide（決定）・Perform（実行）は担い手を明確にするための区分であり、どの役割がいつ動くかは意思決定の性質によって変わります（提案と情報提供が何度も往復することもあります）。RAPIDの最大の価値は、**「Decide（最終決定者）」を必ず1つに絞ること**と、**「Agree（限定的拒否権）」と「Input（単なる助言）」を明確に区別すること**にあります。これにより、「全員が決めようとして誰も決まらない」状態と、「誰も決めずに漂流する」状態の両方を防ぎます。

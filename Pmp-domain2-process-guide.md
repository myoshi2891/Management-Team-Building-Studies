...
試験問題は、この構造に沿って各Domainの全Taskをカバーするように出題され、PMIはDomainレベルでの出題比率(33/41/26%)を遵守しています。したがって、Domain II内の10Taskはどれも「出やすい/出にくい」の差が小さく、まんべんなく学習する必要があります。
...
Domain IIの中でも「アジャイル/価値駆動」の考え方が最も色濃く反映されたTaskです。単に成果物を作ることではなく、ビジネス価値を実際に届けることに焦点を当てます。
...
> **ベストプラクティス**
> - 契約タイプの選定は「リスクを誰が負うか」を軸に考える(要件が不明確なら固定価格は避ける、など)
...
- 出題の約60%は**アジャイル・ハイブリッドのアプローチ**を含むため、予測型の知識だけでは不十分です。Task3(価値ベースデリバリー)やTask8(開発アプローチに応じたスケジューリング)は特にアジャイルの考え方と直結しています。

...
> 本ガイドの立ち位置: 第1部・第2部は Scrum Alliance 公式サイトおよび認定トレーニングパートナーの公開情報に基づく事実整理です。第4部の「ベストプラクティス」は、公式に明記されている4つの学習領域(Learning Objectives)の名称・範囲を土台にしつつ、アジャイルリーダーシップ領域で一般的に参照される実践知(コーチング、チェンジマネジメント、組織文化論など)を組み合わせた実務解説です。両者を混同しないよう、事実部分には出典番号 `[n]` を付けています。巻末の「参考文献・ソース一覧」で確認できます。
...
出典: [9]。この再編は、市場調査・トレーナーや受講生からのフィードバックをもとに、より一貫性のある学習パスを提供する目的で行われました [9]。
...
CAL 1 の学習目標(Learning Objectives)は、公式に **4つの領域** に分類されています [7]。この4領域は、「個人の内省 → 個人の実践 → チームへの適用 → 組織への適用」という流れで構成されており、Day 1 は主に前半2領域(個人にフォーカス)、Day 2 は後半2領域(チーム・組織にフォーカス)に対応します [7]。
...
以下、各領域を初学者向けに詳しく解説します。**「学習目標の範囲」は出典[7]の公式情報に基づく事実、「ベストプラクティス」は一般的なアジャイルリーダーシップ実践知に基づく補足解説**として区別しています。
...
多くのトレーニングパートナーが公開している標準的な2日間構成は、以下のような流れです [13]。
...
なお、多くのトレーニングパートナーでは受講開始の約2週間前に事前課題(所要時間の目安は約1時間)が送付され、これを完了しておくことが推奨されています [13]。
...
| 項目 | 内容 |
|---|---|
| 定義 | 継続的な学習活動(講座受講、記事講読、イベント参加、ボランティアなど)1時間 = 1 SEU |

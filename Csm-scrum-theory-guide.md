...
2020年版のScrum Guideでは、3本柱に加えて「信頼（Trust）」という概念が価値観のセクションに登場します。「これらの価値観がスクラムチームと関係者に体現されたとき、経験主義の3本柱（透明性・検査・適応）が息を吹き込まれ、信頼が築かれる」という趣旨の記述です（出典: 1, 3）。Scrum.orgのブログでも、この「信頼」を経験主義を支える隠れた第4の要素として論じています（出典: 13）。試験対策上は「3本柱＝透明性・検査・適応」という基本を最優先で押さえつつ、信頼が価値観と3本柱をつなぐ土台であることも理解しておくと理解が深まります。

...
| フィールド | 値 |
|---|---|
| コードコミット HEAD | `d0542a8` — feat(psm): complete sections 11-13, footer, styling and source parity（本ファイルのコミットより前のコード側コミット） |
...
- **カタログ体系への追加**: `app/utils/guide-catalog.ts` に新シリーズ `so-scrum-master`（「スクラムマスター」/ "SCRUM MASTER"）を追加。Scrum.org 認定プログラム内のガイドとして登録した。

...
| フィールド | 値 |
|---|---|
| コードコミット HEAD | `17d3e82` — refactor(a-csd): archive original html and md files and update audit script path（本ファイルのコミットより前のコード側コミット） |
...
完了済み:
  ...
  - ユニットテスト 1255 件 / E2E 20 件（全ページ x 3 幅の横スクロール禁止ゲートを含む）
  - test / typecheck / lint はいずれも 2026-08-30 時点で ✔（実測）
  - 全ページ型検査 (nuxi typecheck) / リンター (eslint) / 原本照合監査は全 48 本 exit 0 パス

ベースラインテスト数: ユニット 1255 + E2E 20
```

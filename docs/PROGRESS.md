...
| フィールド | 値 |
|---|---|
| コードコミット HEAD | `f53596f` — chore(archive): move SPS html and md sources to archive（本ファイルのコミットより前のコード側コミット） |
...
完了済み:
  ...
  - ユニットテスト 1706 件 / E2E 29 件（全ページ x 3 幅の横スクロール禁止ゲートを含む）
  ...
  - 全ページ型検査 (nuxi typecheck) / リンター (eslint) / 原本照合監査は全 64 本 exit 0 パス

ベースラインテスト数: ユニット 1706 + E2E 29
```

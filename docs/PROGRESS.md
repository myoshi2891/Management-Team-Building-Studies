...
| フィールド | 値 |
|---|---|
| コードコミット HEAD | `32d5ead` — test(nav): 現在地表示の期待値をハブ方式へ追随させる（本ファイルのコミットより前のコード側コミット） |

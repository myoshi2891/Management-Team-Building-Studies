...
| フィールド | 値 |
|---|---|
| コードコミット HEAD | `8e52926` — fix(psk): remove duplicate border and padding from diagram wrapper（本ファイルのコミットより前のコード側コミット） |
...
完了済み:
  ...
  - ユニットテスト 1677 件 / E2E 29 件（全ページ x 3 幅の横スクロール禁止ゲートを含む）
  ...
  - 全ページ型検査 (nuxi typecheck) / リンター (eslint) / 原本照合監査は全 63 本 exit 0 パス

ベースラインテスト数: ユニット 1677 + E2E 29
```

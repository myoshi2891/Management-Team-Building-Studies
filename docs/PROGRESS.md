...
| フィールド | 値 |
|---|---|
| コードコミット HEAD | `7e41ecc` — style(pal-i): remove max-width constraint from main-content to allow full width（本ファイルのコミットより前のコード側コミット） |
...
完了済み:
  ...
  - ユニットテスト 1594 件 / E2E 29 件（全ページ x 3 幅の横スクロール禁止ゲートを含む）
  ...
  - 全ページ型検査 (nuxi typecheck) / リンター (eslint) / 原本照合監査は全 60 本 exit 0 パス

ベースラインテスト数: ユニット 1594 + E2E 29
```

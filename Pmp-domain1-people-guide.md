...
- **表記方針**: 日本語を主体としつつ、PMI公式の英語術語(Task名・Enabler名・フレームワーク名など)はそのまま残しています。試験問題は英語で作成された問題を各国語に翻訳する形式のため、英語術語に慣れておくことが得点力に直結します。
...
Domain I: Peopleは、プロジェクトを率いる上で最も比重の大きい領域(33%)であり、「人・チーム・ステークホルダーとどう向き合うか」という、プロジェクトマネージャーのリーダーシップとソフトスキルを問う領域です。8つのTaskで構成されています。

```mermaid
flowchart TD
    D["Domain I: People (33%)"]
    D --> T1["Task 1<br/>共通ビジョンの醸成"]
    D --> T2["Task 2<br/>コンフリクトのマネジメント"]
    D --> T3["Task 3<br/>プロジェクトチームのリード"]
    D --> T4["Task 4<br/>ステークホルダーのエンゲージメント"]
    D --> T5["Task 5<br/>期待値の整合"]
    D --> T6["Task 6<br/>期待値のマネジメント"]
    D --> T7["Task 7<br/>知識移転の推進"]
    D --> T8["Task 8<br/>コミュニケーションの計画"]
...
flowchart LR
    ...
    classDef step fill:#EAF4EC,stroke:#2F6B3D,color:#161B26;
...
一般に、PMP試験では「Collaborating(協調)」が最も好ましい選択肢として扱われる傾向がありますが、状況によっては他のモードが正解になることもあります。試験対策としては「どのモードが常に正しいか」ではなく「この状況ではなぜこのモードが適切か」を状況ごとに判断できるようにしておくことが重要です。
...
プロジェクトを通じて蓄積される知識が、個人の頭の中に留まらず組織の資産として継承されるよう促進するTaskです。ECO本文中で構造の例示として使われているTaskでもあり、出題頻度・重要度の高さがうかがえます。
...
```mermaid
flowchart LR
    A["プロジェクトに重要な<br/>知識の特定"] --> B["知識の収集"]
    B --> C["形式知への変換"]
    C --> D["知識移転を促す<br/>環境づくり"]
    D --> E["組織のナレッジベースへ格納"]

    classDef step fill:#FAF1DF;
    style A fill:#FAF1DF,stroke:#B8802A,color:#161B26
    style B fill:#FAF1DF,stroke:#B8802A,color:#161B26
    style C fill:#FAF1DF,stroke:#B8802A,color:#161B26
    style D fill:#FAF1DF,stroke:#B8802A,color:#161B26
    style E fill:#EAF4EC,stroke:#2F6B3D,color:#161B26
```
...
flowchart TD
    ...
    classDef method fill:#EEF1F8,stroke:#2E3F72,color:#161B26;
    class C,D,E method;
```

...
```mermaid
flowchart TD
    T1["Task1 アダプティブアプローチが適切な場面を説明する"] --> T2["Task2 プロジェクトイテレーションの計画方法を決定する"]
    T2 --> T3["Task3 アダプティブプロジェクトのプロジェクトコントロールの文書化方法を決定する"]
    T3 --> T4["Task4 アダプティブ計画の構成要素を説明する"]
    T4 --> T5["Task5 タスク管理ステップの準備・実行方法を決定する"]
```
...
flowchart TD
    ...
    EPIC --> STORY["ユーザーストーリー"]
    STORY --> TASK["タスク 見積り可能な作業単位"]
    TASK --> PBI["プロダクトバックログアイテム PBIとして一覧化"]
    PBI --> REFINE["優先順位付け バックログリファインメント"]
...
Scrumは、複雑な問題に対応するためのシンプルな**フレームワーク**(方法論そのものではなく、その上に実践を積み重ねる土台)です。3つのロール、5つのイベント、3つの成果物から構成されます。

| 構成要素 | 内容 |
|---|---|
| ロール(Accountabilities) | プロダクトオーナー(価値を最大化する責任)、スクラムマスター(Scrumの理解と実践を促進する責任)、開発者(Developers、増分を作成する責任) |

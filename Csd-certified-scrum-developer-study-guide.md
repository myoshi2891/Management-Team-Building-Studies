...
CSDは「Developer Track（開発者トラック）」の入門資格であり、スクラムフレームワークの理解に加えて、**アジャイルなソフトウェアエンジニアリングプラクティス**（リファクタリング、テスト駆動開発、継続的インテグレーションなど）の実践力を証明する認定です。単なる知識試験ではなく、Scrum Alliance認定トレーナーによる**14時間以上のライブ形式トレーニング**と実技評価を通じて取得します。

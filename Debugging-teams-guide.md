...
多くの書評で共通して語られているのは、**チーム内の社会的な対立を読み解く視点として、この3つの欠如を疑うと有効な場合が多い**という考え方です。もっとも、チームの問題はプロセス設計、目標や利害の不一致、権限・力関係といった構造的な要因から生じることもあり、HRTだけですべてを説明できるわけではありません。何か問題が起きたときは、「謙虚さ・敬意・信頼のうち、どれが不足しているのか」を自問することを、原因分析の有力な切り口の一つとして持っておくとよいでしょう。
...
まず最初に取り組むべきは、自分自身の思考の癖を変えることです。多くの技術者は「一人で優れたコードを書ける天才こそ理想」という無意識の思い込み（いわゆる天才神話）を持っています。しかし、コードを隠して一人で抱え込むことは、レビューの機会を失い、バス係数（＝重要な知識を持つメンバーが何人欠けたらプロジェクトが停止または大幅に遅延するか、その人数）を下げてしまうだけです。
...
リーダーの中心的な仕事は、作業手順を細かく指示することではなく、チームの障害物を取り除き、メンバーが自律的に動けるよう支援することです。書籍では、この考え方を「サーバントリーダー（奉仕型リーダー）」と呼んでいます。

ここで押さえておきたいのは、**サーバントリーダーシップはマネジメントの否定でも代替でもない**という点です。目標の設定、期待値の擦り合わせ、評価とフィードバック、要員計画、リスクへの対処といった管理責任は、リーダーが引き続き担います。サーバントリーダーシップが変えるのは「その責任をどう果たすか」であり、細かな指示と監視ではなく、障害物の除去・支援・権限委譲を通じてチームの自律性を高める形で果たす、という関係になります。

したがって下表は「マネジメントをするかしないか」の対比ではなく、**同じ管理責任に対する2つの果たし方**の対比として読んでください。

| 観点 | 指示統制型のマネジメント | サーバントリーダーシップ |
|---|---|---|
| 関心の中心 | 物事をどう終わらせるか（How） | 何を成し遂げるべきか（What） |
| 期待値の伝え方 | 作業手順を指示し、進捗を細かく確認する | 目標と判断基準を共有し、進め方は委ねる |
| 管理責任の果たし方 | 監視・統制によって果たす | 障害物の除去・支援・権限委譲によって果たす |
...
> 本ガイドは上記出典の内容を筆者の言葉で要約・再構成したものであり、原文からの逐語的な引用は最小限にとどめています。なお、引用が適法となる範囲は準拠法域、引用の目的、引用量とその重要性、および出典のライセンス条件によって判断されるものであり、一律の語数基準が存在するわけではありません。より詳しい内容や原文のニュアンスを確認したい場合は、上記リンク先の一次情報を直接ご参照ください。

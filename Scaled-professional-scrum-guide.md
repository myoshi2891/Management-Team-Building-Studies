...
> 世界トップクラスのソフトウェアエンジニア兼スクラムマスターとして、初学者にも分かるようにゼロから解説します。本ガイドは Scrum.org 公式の Nexus Guide、公式アセスメントページ、および公式ブログ・ホワイトペーパーの内容を基に、独自の言葉で体系化したものです。引用は最小限にとどめ、一次情報源への参照 URL を各章末に明記しています。
...
| 出題形式 | 選択式・複数選択・True/False 混在、約40〜80問(バージョンにより変動)、60分 |

...
| 前提資格 | Scrum Alliance承認コースの受講のみ | **CSD®(有効/失効いずれでも可)、またはPSM IもしくはPSM IIの保有が必須** |
...
flowchart TD
    ...
    R1 -- いいえ --> G1["CSD®コースを受講してCSD®を取得する<br/>または PSM I / PSM II を取得する<br/>(どちらか一方でよい)"]
    G1 --> R1
...
1. **CSD®またはPSM I / PSM IIの保有**: 有効なCSD®でも、失効したCSD®でも要件を満たします。CSD®を持たない場合は、Scrum.orgのPSM IまたはPSM IIの保有でも前提資格を満たせます。CSD®で要件を満たした場合、A-CSD®取得と同時にCSD®は自動的に更新されます。
...
- **更新は2年サイクル**: A-CSD®は2年ごとに更新が必要です。通常の更新経路は**SEUの記録の提出と更新料の支払い**の両方を行うことなので、カレンダーにリマインダーを設定しておくことがベストプラクティスです。
- **もう1つの更新経路(認定コースによる自動更新)**: SEUの提出だけが唯一の方法ではありません。Scrum Allianceの認定コースを受講して新たな認定を取得した場合、保有する既存の認定は自動的に更新されます。これは同一トラックの上位認定に限らず、別トラックの認定コース(例: 開発者トラックの保有者がCSPO®コースを受講する場合)であっても更新経路になります(同一トラックの例: A-CSD®を取得するとCSD®が、CSP-D®を取得するとCSD®・A-CSD®が更新される)。次に受講したいコースがあるなら、更新タイミングと受講時期を合わせると無駄がありません。

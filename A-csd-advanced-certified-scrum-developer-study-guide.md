...
- **もう1つの更新経路(認定コースによる自動更新)**: SEUの提出だけが唯一の方法ではありません。同じトラックの上位認定コースを受講して新たな認定を取得した場合、既存の認定は自動的に更新されます(例: A-CSD®を取得するとCSD®が、CSP-D®を取得するとCSD®・A-CSD®が更新される)。上位認定を目指す予定があるなら、更新タイミングとコース受講時期を合わせると無駄がありません。

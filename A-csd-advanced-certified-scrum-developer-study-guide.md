...
| 引き継ぎ(Handoffs) | 開発者からQAへの引き継ぎ待ち | ペアプログラミングや自動テストで引き継ぎ自体を減らす |
| 遅延(Delays) | コードレビュー待ち、CI待ち | 「レビューは24時間以内」のようなSLAをチーム規約にする |
| 欠陥(Defects) | バグの後戻り作業 | 「自動テストがグリーン」をDoDの必須条件にする |
| タスクの切り替え(Task Switching) | 複数プロジェクトの掛け持ち | WIP制限を導入する |

...
4. [フェーズ0:就任前の準備(Day -14〜-1)](#4-フェーズ0就任前の準備day--14-1)
...
まず全体像を掴みましょう。このロードマップは、**就任前の準備期間(フェーズ0、Day -14〜-1)** と **就任後の60日間(フェーズ1〜3、Day 1〜60)** の2部構成です。
つまり全体としては「就任前14日 + 就任後60日間」を扱います。60日間そのものが4つに分かれるわけではなく、Day 1 より前にフェーズ0の準備期間が置かれている点に注意してください。

> **日付の数え方**: 本ガイドでは **Day 0 を「正式な就任日」そのもの** と定義します。準備期間(フェーズ0)は就任前日の **Day -1 で終わり**、就任後のカウントは翌 **Day 1 から**始まります。つまり Day 0 は準備期間にも60日間にも含まれない「境界の日」です。
...
flowchart TB
    ...
    subgraph Phase0["フェーズ0: 就任前の準備 (Day -14〜-1)"]
        direction TB
        P0A["現状把握のための情報収集"]
        P0B["前任者・上司との対話"]
        P0C["初日メッセージの準備"]
...
| フェーズ | 期間 | 一言でいうと | 主な問い |
|---|---|---|---|
| フェーズ0 | 就任前(Day -14〜-1) | 助走をつける | 何を知っておくべきか? |
...
| 記録 | 話した内容と次のアクションを**必要最小限**だけメモし、次回参照できるようにする(共有範囲・保存場所・保存期間・アクセス権をあらかじめ決めて明示する) |
| 頻度の調整 | 慣れてきたら、メンバーの状況に応じて頻度・スタイルを調整する |

> **1on1記録の取り扱いルール**: 1on1のメモは、メンバーが安心して話せることを前提に成り立っています。
> 記録を残す場合は、(1)何を書き何を書かないか、(2)誰が読めるか(共有範囲)、(3)どこに保存するか、
> (4)いつまで保存し、いつ削除するか、(5)アクセス権を誰が管理するか、をチームに対して明示してください。
> 評価・人事・健康に関わる内容や、本人の同意なく共有できない事柄は、個人メモではなく
> 所定の人事プロセスと保管先で扱います。
...
Googleが2012年から実施した大規模調査「Project Aristotle」は、180のチームを対象に、250以上の調査項目(チームの属性)を分析しました。その結果、**チームの効果性と最も強く関連していたのは「心理的安全性」であり、それは「誰がチームにいるか」というチーム構成よりも「チームがどのように協働しているか」に関わる要因だった**と報告されています(相関として報告されたものであり、因果を断定したものではありません)。心理的安全性とは、Harvard Business SchoolのAmy Edmondson教授が定義した「対人関係においてリスクを取っても安全だとチームメンバーが共有して信じている状態」を指します。
...
上記の活用例のうち「難しい対話のロールプレイ」と「1on1の準備」は、本人の同意なく共有できない情報を扱いがちです。次のルールを守ってください。

- **承認されていない外部AIサービスに入力しない情報**: 個人を特定できる情報(氏名・所属・役職の組み合わせなど)、その他の個人情報、評価・人事に関する情報、健康に関する情報、会社の機密情報
- **匿名化してから使う**: 相談したい状況は「シニアエンジニア1名」のように役割と事象だけに抽象化し、個人が特定できる要素を落としてから入力する
- **利用するサービスを確認する**: 会社が承認したAIサービスかどうか、入力データの保存期間、モデルの学習に利用されるか、会話履歴を自分で削除できるかを事前に確認する。学習利用をオフにできる場合はオフにする
- **確認できないサービスには入力しない**: 上記3点(保存期間・学習利用・削除可否)を確認できないサービスには、匿名化した情報であっても入力しない。一度送信した情報は取り消せない前提で扱う
- **残さない**: 準備が終わったら、AIサービス側に残った会話履歴は削除する。ただし削除はあくまで事後対応であり、これ自体がプライバシー保護の担保にはならない(削除前にバックアップや学習に取り込まれている可能性がある)

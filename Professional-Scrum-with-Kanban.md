...
4. [スクラムイベントにおけるカンバンの適用](#4-スクラムイベントにおけるカンバンの適用)
...
WIP制限を設ける最大の目的は、**同時に進行させる作業の数を意図的に絞ることで、個々のアイテムのCycle Timeを短縮し、ボトルネックを可視化すること**です。これはリトルの法則（[3-5章](#3-5-リトルの法則little's-law)参照）に基づく数学的な帰結でもあります。
...
| スクラムマスターがWIP制限を一方的に決定する | WIP制限を含むワークフローの定義（DoW）は、開発を行うDevelopersが主体的に定め、継続的に検査・適応する対象である |

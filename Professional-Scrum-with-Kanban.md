...
- 終了点：スクラムチームが Definition of Workflow の中で「Finished」として定義した瞬間（「完了の定義（Definition of Done）」を満たした時点やステークホルダーに価値が届いた時点を採用するチームが多いが、どこを終了点とするかはチームが決める）
...
- 平均値が有限かつ定義可能であること（平均が未定義または発散する場合にのみ、法則そのものの適用条件に影響する）

なお、**極端な外れ値（非常に長期間放置されたアイテムなど）は法則自体を無効にしません**。ただし観測される平均値や予測の精度を大きく歪めるため、データ品質上の注意点として外れ値の影響は必ず点検してください。
...
緊急性の高い作業のために「特急レーン（Expedite Lane）」を設けるチームは多いですが、運用ルールを誤るとWIP制限の意味そのものを壊してしまいます。以下は The Kanban Guide for Scrum Teams が定める規則ではなく、チームが自分たちのワークフローの定義（DoW）として採用できる**ポリシーの例**です。実際の上限や基準はチームが自ら決め、検査と適応の対象とします。

- **ポリシー例1**：Expediteレーンに入れられるアイテムの上限を定める（「常に1件まで」とするチームが多い。同時に複数の「緊急」は存在しえない、という合意を明文化する）
- **ポリシー例2**：Expediteレーンに入れる基準を明文化し、安易に使われないようにする（例：本番障害のみ、など）
- **ポリシー例3**：Expediteレーンのアイテムも、システム全体のWIPの一部としてカウントする。「特急だから制限外」という扱いにしない
- **ポリシー例4**：Expediteレーンの使用頻度をレトロスペクティブで振り返り、頻発するようであれば根本原因（プロセスの欠陥や品質問題）に対処する
...
flowchart LR
    ...
    subgraph 特急レーン
        direction LR
        E1["緊急発生"] --> E2["Expedite<br/>上限はチームが定める<br/>（例：常に1件まで）"] --> N3
    end
```
...
| WIP制限を超えて、緊急タスクは無制限にレーンへ入れてよい | Expediteレーンの扱いは、チームがワークフローの定義（DoW）で定めるポリシーに依存する。多くのチームは「Expediteアイテムもシステム全体のWIPに算入する」「同時1件まで」といった上限をポリシーとして明示しており、無制限の許容を自チームのポリシーとして選ぶとWIP制限の目的（Cycle Timeの短縮）が崩れる |
...
1. **The Kanban Guide for Scrum Teams と Scrum Guide を繰り返し精読する**：PSK I の設問は The Kanban Guide for Scrum Teams に加えて Scrum Guide の内容も根拠としているため、一字一句の定義（特に4つのフローメトリクスとカンバンプラクティスの名称・定義）を正確に覚えたうえで、それらをスクラムの文脈の具体的なシナリオに当てはめて判断できるようにしておくことが最優先です。定義の暗記だけでは、状況に対する最善の対応を選ぶ設問に対応できません。

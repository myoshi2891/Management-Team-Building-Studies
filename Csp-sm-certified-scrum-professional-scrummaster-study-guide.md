...
> **ベストプラクティス**: 複数チームのバックログを構造化する際は、まず「なぜ複数チームなのか(=1チームでは扱いきれないスコープなのか)」を確認したうえで、依存関係を可視化するワークショップ（例: Nexus の Sprint Backlog のようなクロスチーム依存の可視化）を定期的に実施することが推奨されます。

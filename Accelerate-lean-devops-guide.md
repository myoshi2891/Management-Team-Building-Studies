...
| 継続的な改善（改善） | 一度作って終わりではなく、恒常的にプロセスを見直し続ける |
...
| 2023年 | MTTRを「デプロイ後の障害復旧時間」に名称変更（変更起因の障害と外部要因を分離するため） |
...
| デプロイ後の障害復旧時間 | 障害発生から復旧までの所要時間（旧称: MTTR） | スループット |
...
- [ ] 自チームの現状（デプロイ頻度・リードタイム・変更失敗率・障害復旧時間・リワーク率）を把握した

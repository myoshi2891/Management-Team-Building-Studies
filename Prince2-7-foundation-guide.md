...
**⑤ 例外による管理**
- トレランス(Tolerance)は6つの目標領域で設定される:**時間(Time)・コスト(Cost)・品質(Quality)・スコープ(Scope)・リスク(Risk)・便益(Benefit)**
...
- **トレランス(6領域):** 時間・コスト・品質・スコープ・リスク・便益(原則⑤参照)
- **統制の種類:**
...
- サステナビリティに関する目標・制約を、品質基準やトレランス設定に反映する。
...
3. **トレランスの6領域(時間・コスト・品質・スコープ・リスク・便益)** を即答できるようにする。

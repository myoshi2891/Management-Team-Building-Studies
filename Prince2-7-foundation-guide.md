...
**④ 段階による管理**
- 初期化ステージ+提供ステージという構成は一般的な例であり、必須のステージ数が定められているわけではない。目的達成に必要な統制を維持しながら、プロジェクトの規模・リスクに応じてステージ構成を調整する。
...
flowchart TB
    ...
    BL --> BL7["プロジェクト・プロダクト・<br/>ディスクリプション"]
    BL --> BL8["ワークパッケージ・<br/>ディスクリプション"]
    BL --> BL5["プロジェクトブリーフ"]
    ...
    REP --> REP6["課題報告<br/>(Issue Report)"]
    REP --> REP7["教訓報告<br/>(Lessons Report)"]
```

> **補足(用語の変遷とV7での新設):** 「戦略(Strategy)」から**「マネジメント・アプローチ(Management Approach)」**への改称はV6(2017年版)で行われ、V7では引き続きこの呼称が使われている。V7ではこれに加えて、**チェンジ・マネジメント・アプローチ**（既存製品の改称ではなく、組織変革の実現に焦点を当てて新設されたマネジメント・プロダクト）、**コマーシャル・マネジメント・アプローチ**、**デジタル&データ・マネジメント・アプローチ**、**サステナビリティ・マネジメント・アプローチ**の4つが新設された。あわせて、V6の「チェンジ・コントロール・アプローチ」（Changeテーマに属し、変更管理の手続きを定めていた）は、V7では**「イシュー・マネジメント・アプローチ」**（Issuesプラクティス。リスクとは別に、問題・懸念、ビジネス機会、変更要求、仕様逸脱などの課題を扱い、必要なベースライン変更を統制する課題管理として再構成）に置き換えられている。
...
- **兼務してはいけない組み合わせの代表例:**
  ...
  - プロジェクトアシュアランス(独立したチェック機能)は、プロジェクトマネージャー・チームマネージャー・プロジェクトチームメンバー・プロジェクトサポートのいずれにも割り当てられない(自らの仕事を自ら監査することになるため)
  - エグゼクティブの説明責任(アカウンタビリティ)自体は委譲できない
- **兼務とは区別すべき「委任」:** プロジェクトボードは、保証活動そのものを適任者や外部の第三者へ委任できる。ただし委任した後も、その保証活動に対する説明責任は該当するプロジェクトボードメンバー(ビジネス/ユーザー/サプライヤーの各利害を代表する者)が引き続き負う。これは「役割を他者と兼ねる」話ではなく「実施を他者に任せても責任は残る」という話である
...
1. **用語の日英対応を正確に覚える。** 特にV6→V7の改称(Themes→Practices、Change→Issues)と、V5→V6の改称(Strategy→Management Approach)は誤答を誘発しやすい。

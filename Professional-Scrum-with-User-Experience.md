...
| ステークホルダー（Stakeholder） | プロダクトの成功に利害関係を持つが、日々の開発には関与しない人・組織 | 経営層、営業部門、法務、サポート部門 |
...
重要なポイントは、**Dual-Track AgileはScrumのイベントを増やしたり複雑にしたりするものではない**ということです。DiscoveryとDeliveryは「別のプロセス」ではなく、**同じProduct Backlogに合流する、並行した2つの作業の流れ**です。Discoveryで検証された結果が、Product Backlogアイテムとして磨き込まれ、Sprint Planningでピックアップされてはじめて実装に進みます。
...
C. 何を作るべきかを検証するDiscoveryと、それを実装するDeliveryを並行させ、検証済みの作業のみがバックログに積まれるようにするため
D. UXデザイナーを開発チームから独立させ、別々のバックログで管理するため

<details><summary>解答と解説</summary>

**正解: C**
Dual-Track Agileは、DiscoveryとDeliveryを並行させることで、Deliveryトラックが常に検証済み・優先順位づけされた作業に取り組めるようにする考え方です。ScrumのイベントやArtifactを増やすものではなく、同じProduct Backlogに合流します。
</details>
...
B. ステークホルダーはプロダクトの成功に利害関係を持つが日々の開発には関与しないことが多く、顧客・ユーザーとは区別して理解する必要がある
...
**正解: B**
ステークホルダー、顧客、ユーザーは重なることもありますが、常に同一とは限りません。特にBtoBプロダクトでは、契約する顧客と実際に使うユーザーが異なることが多く、これらを区別して意思決定することが重要です。
</details>

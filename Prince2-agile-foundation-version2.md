...
> **出典:** PeopleCert 公式製品ページ「PRINCE2 Agile Foundation (Version 2)」<br>
...
```mermaid
flowchart LR
    F["PRINCE2 Agile Foundation (Version 2)"] --> P["PRINCE2 Agile Practitioner (Version 2)"]
    P --> E["PRINCE2 Agile Expert (Foundation + Practitioner 取得で到達)"]
```
...
```mermaid
flowchart TB
    DP["Directing a Project (プロジェクト指揮)"]
    SU["Starting up a Project (立ち上げ)"] --> IP["Initiating a Project (開始)"]
    IP --> CS["Controlling a Stage (ステージ管理)"]
    CS --> MP["Managing Product Delivery (プロダクト提供管理)"]
    MP --> CS
    CS --> SB["Managing a Stage Boundary (ステージ境界管理)"]
    SB --> CS
    SB --> CP["Closing a Project (プロジェクト終結)"]
    DP --- SU
    DP --- IP
    DP --- CS
    DP --- SB
...
```mermaid
flowchart TB
    subgraph L1["プロジェクト指揮 (Project Direction)"]
        A1["PRINCE2: プロジェクトボードによるガバナンスと意思決定"]
    end
    subgraph L2["プロジェクトマネジメント (Project Management)"]
        A2["PRINCE2: プロセスとテーマによる管理の枠組み"]
        A3["Agile: タイムボックス化された計画とレビュー"]
    end
    subgraph L3["プロダクトデリバリー (Product Delivery)"]
        A4["Agile: Scrum / Kanban によるチームの日々の実行"]
    end
    L1 --> L2
    L2 --> L3
```
...
```mermaid
flowchart LR
    TODO["To Do"] --> DOING["Doing (WIP制限あり)"]
    DOING --> REVIEW["Review"]
    REVIEW --> DONE["Done"]
```
...
```mermaid
flowchart TB
    FA["4つのフォーカスエリア"] --> AG["Agilometer"]
    FA --> RQ["Requirements (要求事項の優先順位付け)"]
    FA --> RC["Rich Communication"]
    FA --> FR["Frequent Releases"]
```
...
- 40問の四肢択一式、60分、クローズドブック、合格ラインは60%(0.4節参照)。
...
```mermaid
flowchart TB
    S1["Step 1: 第1部で PRINCE2 の土台(原則・テーマ・プロセス・管理プロダクト・ロール)を固める"] --> S2
    S2["Step 2: 第2部で PRINCE2 Agile 固有の考え方(ブレンドとウィーブ、8つのガイダンスポイント)を理解する"] --> S3
    S3["Step 3: 第3部で原則・テーマ・プロセス・管理プロダクトのアジャイル適用を、対応表として覚える"] --> S4
    S4["Step 4: 第4部で用語と技法を、実際の文脈の中で使えるレベルまで定着させる"] --> S5
    S5["Step 5: 第5部のフォーカスエリアを押さえ、模擬問題でシナリオ形式に慣れる"]
```
...
> **出典:** 本節は公式シラバスの Learning Outcome 全体(1〜5)を横断する形で、頻出論点を整理したものです。<br>
> https://www.koenig-solutions.com/CourseContent/custom/2019617460-PRINCE2AgileFoundationCandidateSyllabus.pdf
...
- What Is PRINCE2「PRINCE2 Agile Book Review」<br>

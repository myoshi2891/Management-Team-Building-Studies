...
Professional Scrum Product Owner（PSPO）は、Scrum.org が提供する認定試験の中でも「プロダクトの成功責任者」としての知識とマインドセットを問う試験群です。レベルは PSPO I / PSPO II / PSPO III の3段階があり、本ガイドではこのうち PSPO I と PSPO II を対象とします。

| 項目 | PSPO I | PSPO II |
|---|---|---|
| 対象レベル | 基礎〜実務初級 | 実務中級〜上級（戦略・組織視点） |
| 出題形式 | Multiple Choice（単一選択）/ Multiple Answer（複数選択）/ True or False | Multiple Choice / Multiple Answer（一部の問題は部分点あり）。シナリオベースでより深い考察を要求 |
| 問題数の目安 | 80問 | 40問 |
| 制限時間の目安 | 60分 | 60分 |
| 合格ラインの目安 | 85% | 85% |
| 推奨知識・資格 | なし（誰でも受験可） | PSPO I の取得は必須ではないが、PSPO I 相当の知識があることが推奨される |
| 出題の重心 | Scrum Guide の正確な理解、PBL管理、基本用語 | 戦略・EBM・組織スケール・ステークホルダー対応などの応用力 |

PSPO 試験の再受験は原則として有料で、不合格の場合はあらためて受験料が発生します。ただし例外があり、Scrum.org 公式講座の受講者が受講後 14 日以内に PSPO I を受験して不合格となった場合は、追加費用なしで 2 回目を受験できます。いずれにせよ実質的に一発勝負に近い緊張感のある試験設計であるため、丸暗記ではなく Scrum Guide の一次情報に基づいた本質理解が合格への近道になります。
...
これに対して、PSPO が推奨するのは**アウトカムドリブン（ゴール・ドリブン）・ロードマップ**です。時間軸に沿って「達成したい成果」や「解決したい課題」を並べ、その成果を達成する手段（機能）はチームの裁量とディスカバリー活動に委ねます。
...
- **単一のバックログ（Single Source of Truth）**: 1つのプロダクトに対してプロダクトバックログは1つだけ存在する（1プロダクト＝1プロダクトバックログ。複数のスクラムチームが同じプロダクトに取り組む場合も、共有するプロダクトバックログは1つ）。部門ごと・チームごとに別々の優先順位リストを持つことは、優先順位の一貫性を破壊する。
...
プロダクトオーナーは、なぜこのスプリントに価値があるのか（Topic 1: Why is this Sprint valuable?）を説明し、スプリントゴールの草案を提案します。ただし、スプリントゴールを最終的に確定させるのは**スクラムチーム全体**であり、どのPBIをどこまで取り組むかを選び、その届け方の計画を作るのは**開発者（Developers）自身**です。プロダクトオーナーの役割は次の通りです。
...
| **Doneの定義（Definition of Done）** | 組織標準があればそれが最低限。無ければスクラムチームが定義し、同一プロダクトの複数チームは共通のDoneの定義に揃える | リリース可能な品質を保証するための普遍的な基準（例：テストが通っている、コードレビュー済み、ドキュメント更新済みなど） |

インクリメントが「Done」と判断されるためには、**受け入れ基準を満たし、かつ Doneの定義も満たしている**必要があります。片方だけでは不十分です。また、Doneの定義は組織に標準がある場合はそれが最低限の基準となり、標準が存在しない場合はスクラムチームが作成します（開発者はその基準に適合しなければなりません）。プロダクトオーナーが一方的に基準を緩めたり厳しくしたりできるものではない、という点も頻出の論点です。
...
3. **公式のオープンアセスメント（無料の実力診断テスト）を繰り返し受験する**: Scrum.org が提供する Product Owner Open や Evidence-Based Management Open などの無料アセスメントは、本試験の出題傾向を掴む上で非常に有効です。満点近くを安定して取れるようになるまで繰り返すことが推奨されます。

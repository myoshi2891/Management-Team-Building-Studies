...
```mermaid
pie showData
    title Practitioner試験の出題配点
    "LO3: 7つのPractices" : 51
    "LO4: 7つのProcesses" : 30
    "LO1: 7つのPrinciples" : 10
    "LO2: 人(People)の管理" : 9
```
...
```mermaid
flowchart LR
    A["Project Product\nDescription作成"] --> B["Product Breakdown\nStructure作成"]
    B --> C["各Product\nDescription作成"]
    C --> D["Product Flow\nDiagram作成\n(依存関係の可視化)"]
    D --> E["アクティビティと\nリソースの計画"]
```
...
flowchart TD
    A["課題を捕捉\n(Capture)"] --> B["評価\n(Examine): インパクト分析"]
    B --> C{"対応可能な\n許容範囲内か?"}
    C -->|Yes| D["Project Managerが\n対応を決定(Propose/Decide)"]
    C -->|No| E["Project Board /\nChange Authorityへ\nエスカレーション"]
    D --> F["実施\n(Implement)"]
    E --> F
    F --> G["クローズ\n(Close)"]
```
...
```mermaid
flowchart TB
    subgraph PRE["プレプロジェクト"]
        SU["SU: Starting up a Project\n(プロジェクトの立ち上げ)"]
    end
    subgraph INIT["初期ステージ"]
        IP["IP: Initiating a Project\n(プロジェクトの開始)"]
    end
    subgraph DELIVERY["各デリバリーステージ(繰り返し)"]
        CS["CS: Controlling a Stage\n(ステージの統制)"]
        MP["MP: Managing Product Delivery\n(プロダクトデリバリーの管理)"]
        SB["SB: Managing a Stage Boundary\n(ステージ境界の管理)"]
        CS <--> MP
        CS --> SB
    end
    subgraph FINAL["最終ステージ"]
        CP["CP: Closing a Project\n(プロジェクトのクロージング)"]
    end
    DP["DP: Directing a Project\n(プロジェクトの指揮)\n※全期間を通じてProject Boardが実施"]
...
```mermaid
flowchart TD
    A["Quality Management Approach"] --> F["Communication\nManagement Approach\n(最後に確定)"]
    B["Risk Management Approach"] --> F
    C["Issue Management Approach"] --> F
    D["Change Management Approach"] --> F
    E["Sustainability/Commercial/\nDigital & Data Approach"] --> F
```
...
```mermaid
flowchart LR
    A["ワークパッケージ承認"] --> B["Team Managerが実行"]
    B --> C["Checkpoint Reportで報告"]
    C --> D["進捗・課題・リスクを\nレビュー"]
    D --> E{"トレランス内か?"}
    E -->|Yes| F["Highlight Reportで\nProject Boardへ報告"]
    E -->|No/予測される| G["Exception Reportを作成"]
    F --> A
    G --> H["Project Boardの\n判断を待つ"]
```
...
```mermaid
flowchart TB
    subgraph Corporate["Corporate / Programme Management(顧客)"]
        COR["プロジェクトマンデートの発行\n最上位の意思決定"]
    end
    subgraph Board["Project Board(指揮レベル)"]
        EX["Executive\nビジネスケースの所有者"]
        SU_["Senior User\n便益とユーザー要求の代表"]
        SS["Senior Supplier\n技術的実現可能性の代表"]
    end
